(function(){
  ...
  const btn=document.createElement('button');btn.id='lb-btn';btn.textContent='🏆';btn.title='Leaderboards';document.body.appendChild(btn);
  ...
  function visible(){const lock=document.getElementById('lock-overlay');btn.style.display=lock&&getComputedStyle(lock).display!=='none'?'none':'block';}
  async function load(){
