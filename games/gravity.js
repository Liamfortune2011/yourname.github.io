...
var a=setup('gravity-canvas',640,300);if(!a)return;var c=a[0],x=a[1],k=keys(),p,obs,score,best=0,grav,raf,last,clickLock=false;
function stop(){if(raf){cancelAnimationFrame(raf);raf=0}}
function reset(){stop();p={x:90,y:140,vy:0};obs=[];score=0;grav=1;last=performance.now();clickLock=false;by('grav-score').textContent=0;by('grav-msg').textContent='Space or click the game area to flip gravity.';raf=requestAnimationFrame(loop)}
function flip(){if(clickLock)return;clickLock=true;grav*=-1;p.vy=grav*120;setTimeout(function(){clickLock=false},120)}
