...
function slide(i){if(finished||tiles[i]===0)return false;var er=Math.floor(empty/4),ec=empty%4,r=Math.floor(i/4),c=i%4;if(r!==er&&c!==ec)return false;var from=i,to=empty,moving=tiles[from],step;if(r===er){step=from<to?1:-1;for(var p=from;p!==to;p+=step)tiles[p]=tiles[p+step]}else{step=from<to?4:-4;for(var q=from;q!==to;q+=step)tiles[q]=tiles[q+step]}tiles[to]=moving;empty=from;moves++;return true}
