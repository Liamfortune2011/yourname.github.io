...
function slide(i){if(finished||tiles[i]===0||!canSlide(i))return false;var oldEmpty=empty,step;if(Math.floor(i/4)===Math.floor(empty/4)){step=i<empty?1:-1;while(i!==empty){var next=i+step;tiles[i]=tiles[next];i=next}}else{step=i<empty?4:-4;while(i!==empty){var next=i+step;tiles[i]=tiles[next];i=next}}tiles[oldEmpty]=0;empty=i;moves++;return true}
